...
def FE_5(train_, test_):
    ...
    for col in ["총 시술 횟수", "클리닉 내 총 시술 횟수", "IVF 시술 횟수", "DI 시술 횟수", "총 임신 횟수", "IVF 임신 횟수", "DI 임신 횟수", "총 출산 횟수", "IVF 출산 횟수", "DI 출산 횟수"]:
        train[col+"__"] = train[col].apply(lambda x: int(x[0]))
        test[col+"__"] = test[col].apply(lambda x: int(x[0]))
...
save = pd.read_csv("backup_xgb.csv")
start = len(save)
    ...
    try:
        ...
        for fold, (train_idx, valid_idx) in enumerate(kf.split(X, y)):
                ...
                def te(t, v, y, cat):
                    t = t.copy()
                    t["임신 성공 여부"] = y
                    v = v.copy()  # 원본 보호 (경고 방지)
                    
                    for col in cat:
                        means = t.groupby(col)["임신 성공 여부"].mean()
                        t[col] = t[col].map(means)
                        v[col] = v[col].map(means)
                        v[col] = pd.to_numeric(v[col], errors="coerce").fillna(v[col].mean())

                    t = t.drop(columns = "임신 성공 여부")
                    return t, v

                X_train, X_valid = te(X_train, X_valid, y_train, cat_features)
        ...
        save.to_csv("backup_xgb.csv", index=False)
    except:
